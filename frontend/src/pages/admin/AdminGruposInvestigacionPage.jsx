...
import { MdAdd, MdEdit, MdDelete, MdClose, MdCheck, MdGroups, MdSearch } from "react-icons/md";
...
const AdminGruposInvestigacionPage = () => {
  ...
  const filasFiltradas = useMemo(() => {
    ...
    return filas.filter((f) =>
      [f.nombre, f.nombre_corto].filter(Boolean).some((campo) => campo.toLowerCase().includes(term))
    );
  }, [filas, busqueda]);
        ...
        <>
          ...
          <div className="flex items-center justify-between mb-3">
            <p className="text-unmsm-muted text-xs">
              {filasFiltradas.length} {filasFiltradas.length === 1 ? "grupo" : "grupos"}
            </p>
            <ItemsPerPageSelect value={porPagina} onChange={setPorPagina} />
          </div>

          {filasFiltradas.length === 0 ? (
            <p className="text-unmsm-muted text-sm">No se encontraron grupos para "{busqueda}".</p>
          ) : (
                ...
                <div key={fila.id} className="flex items-center justify-between gap-4 p-4">
                  ...
                  <div className="flex items-center gap-1 sm:gap-3 flex-shrink-0">
                    <Link
                      ...
                      className="flex items-center gap-1 text-unmsm-navy hover:text-unmsm-blue text-sm font-semibold p-2 sm:p-0"
                    >
                      <MdGroups className="text-lg sm:text-base" /> <span className="hidden sm:inline">Gestionar integrantes</span>
                    </Link>
                    <button
